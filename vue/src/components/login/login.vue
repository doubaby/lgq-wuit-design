<template>
  <div class="login_container">
    ...
    <span class="back" @click="resetLoginForm">
          <p style="position: absolute;width: 10px;color: #7d8084;right: 80px;top: 10px;font-size: 25px"
             class="el-icon-back"></p>
    <span style="position: absolute;color:#7d8084;right: 18px;top: 10px">上一步</span>
    </span>
  </div>
</template>
      ...
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {//表单校验合法
          login.login(this.user).then(res => {
            if (res.code == 20000) {
              //将token放入cookie
              jsCookie.set("token", res.data, {domain: 'localhost'})
